...
(function (global) {
  ...
  const PROFESSION_PERSONAS = {
    "职场女性": ["都市高智感", "温柔知性风", "松弛高级感", "清透专业感", "轻熟知性风", "摩登都市感"],
    "美妆博主": ["元气甜酷感", "清透韩系感", "复古港风感", "甜酷千金感", "冷艳御姐感", "明媚度假感"],
    "穿搭博主": ["松弛高级感", "法式优雅感", "都市高智感", "轻奢松弛感", "摩登都市感", "艺术策展感"],
    "艺术家": ["清冷电影感", "森系自然感", "复古港风感", "艺术策展感", "文艺书卷感", "东方电影感"],
    "设计师": ["都市高智感", "清冷电影感", "松弛高级感", "艺术策展感", "摩登都市感", "文艺书卷感"],
    "创业者": ["都市高智感", "轻奢名媛感", "清冷电影感", "摩登都市感", "轻奢松弛感", "清透专业感"],
    "老师": ["温柔知性风", "氧气亲和感", "松弛高级感", "文艺书卷感", "清透专业感", "自然原生感"],
    "电商主播": ["元气甜酷感", "轻奢名媛感", "复古港风感", "甜酷千金感", "明媚度假感", "冷艳御姐感"],
    "学生": ["氧气少女感", "元气甜酷感", "森系自然感", "自然原生感", "清透韩系感", "明媚度假感"],
    "宝妈": ["温柔知性风", "松弛高级感", "氧气亲和感", "轻熟知性风", "自然原生感", "法式松弛感"],
    "自由职业": ["森系自然感", "法式松弛感", "清冷电影感", "艺术策展感", "文艺书卷感", "明媚度假感"],
    "银发女性": ["温柔知性风", "轻奢松弛感", "复古电影感", "东方电影感", "自然原生感", "文艺书卷感"],
  };

  const SCENE_PERSONAS = {
    "社交头像": ["清透韩系感", "氧气亲和感", "轻熟知性风", "松弛高级感", "自然原生感", "明媚度假感"],
    "证件照": ["清透专业感", "温柔知性风", "都市高智感", "自然原生感", "轻熟知性风", "松弛高级感"],
    "职场形象照": ["都市高智感", "轻奢松弛感", "清冷电影感", "清透专业感", "摩登都市感", "轻熟知性风"],
    "杂志封面": ["清冷电影感", "法式优雅感", "复古电影感", "摩登都市感", "艺术策展感", "冷艳御姐感"],
    "小红书封面": ["氧气少女感", "元气甜酷感", "松弛高级感", "清透韩系感", "甜酷千金感", "明媚度假感"],
    "日系清冷": ["清冷电影感", "森系自然感", "氧气少女感", "自然原生感", "文艺书卷感", "法式松弛感"],
    "港风穿搭": ["复古港风感", "轻奢名媛感", "清冷电影感", "复古电影感", "冷艳御姐感", "摩登都市感"],
    "国风穿搭": ["新中式清雅感", "温柔知性风", "东方电影感", "艺术策展感", "文艺书卷感", "自然原生感"],
    "新中式穿搭": ["新中式清雅感", "温柔知性风", "东方电影感", "艺术策展感", "文艺书卷感", "自然原生感"],
    "氛围写真": ["松弛高级感", "清冷电影感", "森系自然感", "法式松弛感", "复古电影感", "艺术策展感"],
    "生日写真": ["元气甜酷感", "轻奢名媛感", "氧气少女感", "甜酷千金感", "明媚度假感", "法式优雅感"],
  ...
  function pickPersonaCycle(personas, generationCycle = 0) {
    const start = Math.abs(Number(generationCycle) || 0) % 2 === 0 ? 0 : 3;
    const selected = personas.slice(start, start + 3);
    return selected.length === 3 ? selected : personas.slice(0, 3);
  }

  function selectStylePersonas({ profession, scene, generationCycle = 0 }) {
    const scenePersonas = SCENE_PERSONAS[scene];
    const source = scenePersonas || PROFESSION_PERSONAS[profession] || STYLE_LIBRARY;
    return pickPersonaCycle(source, generationCycle);
  }
  ...
  function buildX3Prompt(choices) {
    ...
    return join(
      ...
      `三张候选风格标签：${personas.join(" / ")}。这 3 个只是候选方案，不要分别标成最佳风格。三张图必须在妆容、发型、穿搭、姿态、背景、光影和气质上明显拉开差异。`,
  ...
  function buildFineTunePrompt(choices, moduleName, tuneValue) {
    return join(
      ...
      `微调模块：${moduleName}。`,
      `微调方向：${tuneValue}。`,
      "要求：保留人物身份、主体构图、文字数量和标签可读性，只调整本次微调方向；如当前画面模板感过强，可在不破坏信息结构的前提下优化为更时尚、更轻盈、更有新鲜感的视觉表达。",
      "禁止：不要把文字、标签或图标压在人像图片上。",
  ...
  function buildDetailedReportPromptFromFirstGeneration(firstGenerationPrompt) {
    return join(
      ...
      "核心要求：保持当前结果图内的人物身份、五官、脸型、发型、妆容、穿搭和主要风格方案一致，不要重新生成成另一个人。",
